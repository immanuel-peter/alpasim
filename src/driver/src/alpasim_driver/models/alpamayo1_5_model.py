...
def _format_trajs(pred_xyz: torch.Tensor) -> np.ndarray:
    """Extract and format trajectory from Alpamayo 1.5 output.
...
class Alpamayo1_5Model(BaseTrajectoryModel):
    """Alpamayo 1.5 wrapper implementing the common interface."""

    # Alpamayo 1.5 uses bfloat16 for inference
    ...
    def from_config(
        ...
        """Create Alpamayo1_5Model from driver configuration."""
        return cls(
    ...
    def __init__(
        ...
        self._model = Alpamayo1_5.from_pretrained(checkpoint_path, dtype=self.DTYPE).to(
            device
        )
        ...
        # Precompute sorted camera IDs and corresponding integer indices for
        # helper.create_message, which labels each image with a camera display
        # name — matching the format used during model training.
        self._sorted_camera_ids = sorted(
            camera_ids, key=lambda c: CAMERA_NAME_TO_INDEX[c]
        )
        self._camera_indices = torch.tensor(
            [CAMERA_NAME_TO_INDEX[c] for c in self._sorted_camera_ids]
        )
    ...
    def _encode_command(self, command: DriveCommand) -> Any:
        """Alpamayo 1.5 reasons about navigation from context, no explicit command encoding."""
        return None
    ...
    def _preprocess_images(self, camera_images: CameraImages) -> torch.Tensor:
        """Preprocess multi-camera images for Alpamayo 1.5.
        ...
        # Process each camera in sorted order (matching self._sorted_camera_ids)
    ...
    def predict(self, prediction_input: PredictionInput) -> ModelPrediction:
        ...
        Alpamayo 1.5 uses camera images and ego pose history. Command, speed,
        ...
        # build_ego_history will fill in the 16 target samples from these poses.
        ...
        # Create chat message using Alpamayo 1.5's helper.
        # camera_indices labels each image with its display name (matching training
        # format); num_frames_per_camera tells the helper how many frames per camera.
        ...
        if "cot" in extra and len(extra["cot"]) > 0:
            ...
            logger.info("Alpamayo 1.5 Chain-of-Causation: %s", reasoning_text)
        else:
            reasoning_text = None
